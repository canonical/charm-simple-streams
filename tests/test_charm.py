...
class TestCharm(unittest.TestCase):
    ...
    def test_synchronize_action_defaults(self, mock_subproc):
        ...
        self.assertTrue(mock_subproc.called)
        assert mock_subproc.call_args == call(['sstream-mirror', '--keep', '--no-verify',
    ...
    def test_synchronize_action_all_params(self, mock_subproc):
        ...
        harness.charm._on_synchronize_action(action_event)
        self.assertTrue(mock_subproc.called)
        assert mock_subproc.call_args == call(['sstream-mirror', '--keep',
                                               '--keyring={}'
    ...
    def test_create_snapshot_action(self, os_makedirs, os_symlink, os_path_exists,
        ...
        self.assertTrue(os_symlink.called)
        assert os_symlink.call_args == call('{}/latest/{}'
                                            .format(default_config['image-dir'], rand_n),
                                            '{}/{}/{}'.format(default_config['image-dir'],
                                                              snapshot_name, rand_n))
        assert shutil_copytree.call_args == call('{}/latest/.data'
                                                 .format(default_config['image-dir']),
                                                 '{}/{}/.data'.format(default_config['image-dir'],
                                                                      snapshot_name))
    ...
    @patch('os.path.exists')
    @patch('os.makedirs')
    def test_create_snapshot_copy_action(self, os_makedirs, os_path_exists,
                                         shutil_copytree, os_walk):
        def a2g(x):
            return ([n, ['{}'.format(n)]] for n in x)
        ...
        action_event = Mock()
        harness.charm._on_create_snapshot_action(action_event)
        assert shutil_copytree.call_args_list[0] == call('{}/latest/{}'
                                                         .format(default_config['image-dir'],
                                                                 rand_n),
                                                         '{}/{}/{}'
                                                         .format(default_config['image-dir'],
                                                                 snapshot_name,
                                                                 rand_n))
        assert shutil_copytree.call_args_list[1] == call('{}/latest/.data'
                                                         .format(default_config['image-dir']),
                                                         '{}/{}/.data'
                                                         .format(default_config['image-dir'],
                                                                 snapshot_name))
    ...
    def test_delete_snapshot_action(self, shutil_rmtree):
        ...
        assert shutil_rmtree.call_args == call('{}/{}'.format(default_config['image-dir'],
                                                              snapshot_name))
    ...
    def test_publish_snapshot_action_success(self, os_unlink, os_symlink, os_path_islink,
        ...
        assert os_symlink.call_args == call('{}/{}'.format(default_config['image-dir'],
                                                           snapshot_name),
